  ...
  return (
    <div
      className={`relative flex flex-col rounded-2xl border p-6 transition hover:-translate-y-1 ${
        isTrial
          ? "border-primary/50 bg-gradient-to-b from-primary/10 to-white/[0.03] shadow-lg shadow-primary/10"
          : plan.popular
    ...
    >
      {isTrial && (
        <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-primary px-3 py-1 text-xs font-semibold text-white shadow">
          Sem Cartão · Sem Risco
        </span>
      )}
      {!isTrial && plan.popular && (
      ...
      {isTrial && (
        <p className="mt-1 text-xs text-white/50">
          Por {plan.trialHours} horas, depois escolha um plano
        </p>
      )}
      ...
      >
        {isTrial ? "Começar Teste Grátis" : "Contratar Agora"}
      </button>
    </div>
  );
}
