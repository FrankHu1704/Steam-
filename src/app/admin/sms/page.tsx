...
type AdminMessage = SmsMessage & { profiles?: { full_name: string; company_name: string | null } }
...
export default function AdminSmsPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/sms?${params.toString()}`)
      .then((r) => r.json())
      .then((res) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">SMS enviados</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400">Todas as mensagens enviadas na plataforma.</p>
      </div>
        ...
        <CardContent>
          <div className="mb-4 flex flex-wrap gap-3">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <Input className="pl-9" placeholder="Pesquisar por número ou mensagem..." value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
            <Select
              className="w-48"
              value={status}
              ...
              <tbody>
                ...
                {messages.map((m) => (
                  <tr key={m.id} className="border-b border-slate-50 last:border-0 dark:border-slate-800/60">
                    <td className="py-2.5 text-slate-700 dark:text-slate-300">{m.profiles?.company_name || m.profiles?.full_name || '—'}</td>
                    <td className="py-2.5 font-mono text-xs text-slate-700 dark:text-slate-300">{m.recipient}</td>
                    <td className="py-2.5 text-slate-600 dark:text-slate-400">{truncate(m.message, 35)}</td>
                    <td className="py-2.5">
